...
def create_vcf_file(data):
    ...
    for _, row in data.iterrows():
        if row['Phone'] and row['First Name']:  # Убедимся, что есть телефон и имя
            vcard = vobject.vCard()
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith('limit_'):
        if query.data == 'limit_custom':
            ...
            await query.message.reply_text("Укажи своё число (максимум 15000):", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(texts['skip'], callback_data='skip_limit')]]))
        else:
